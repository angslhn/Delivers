...
export default function Input({ label, name, onChange, value, forgot_password, invalid }: InputProps): JSX.Element {
  ...
  return (
    <div className="relative flex flex-col h-20 w-full gap-1">
      ...
      {["password", "confirm_password"].includes(name) &&
        (showPassword ? (
          <svg
            ...
            className="absolute right-2 bottom-7 fill-steel-night w-5 hover:fill-steel-night/60"
      ...
      {name === "password" && forgot_password && (
        <Link
          className="absolute w-full bottom-0 right-0 font-semibold text-xs text-right text-steel-night hover:text-steel-night/90"
